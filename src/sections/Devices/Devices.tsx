...
export default async function Devices({
  ...
  description = "Türkiye genelinden tarafımıza gönderilen medikal cihazlara elektronik kart tamiri, mekanik bakım, arıza analizi ve teknik servis desteği sunuyoruz.",
  itemLimit = 6,
  showViewAll = true,
  viewAllLabel = "Tüm Cihaz Gruplarını Gör",
}: DevicesProps) {
  const featuredDevices = await getPublicFeaturedDevices(itemLimit);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {featuredDevices.length ? (
            featuredDevices.map(
              ({ id, title: deviceTitle, shortDescription, capabilities, iconKey }) => (
                <div
                  key={id}
          ...
          <div className="mt-12 overflow-hidden rounded-3xl border border-sky-100 bg-white/85 p-6 shadow-xl shadow-slate-900/5 sm:flex sm:items-center sm:justify-between sm:gap-6 lg:p-8">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                Anasayfada öne çıkan grupları gösteriyoruz. Detaylı cihaz
                kapsamı için cihazlar sayfasına geçebilirsiniz.
              </p>
            </div>
            <Link
              href="/cihazlar"
