...
const requiredText = (min = 1, max = 500) => z.string().trim().min(min).max(max);
...
const optionalCtaUrlSchema = z
  ...
  .trim()
  .max(500)
  .refine((value) => !value || isSafeCtaUrl(value), "Güvenli bir bağlantı girin.")
  .default("");
...
export function parseSiteSettings(raw: Partial<Record<SiteSettingGroup, unknown>>) {
  return {
    general: parseSiteSettingGroup("general", raw.general ?? defaultSiteSettings.general),
    contact: parseSiteSettingGroup("contact", raw.contact ?? defaultSiteSettings.contact),
    whatsapp: parseSiteSettingGroup(
      "whatsapp",
      raw.whatsapp ?? defaultSiteSettings.whatsapp
    ),
    address: parseSiteSettingGroup("address", raw.address ?? defaultSiteSettings.address),
    ...
    social: parseSiteSettingGroup("social", raw.social ?? defaultSiteSettings.social),
    search: parseSiteSettingGroup("search", raw.search ?? defaultSiteSettings.search),
    analytics: parseSiteSettingGroup(
      "analytics",
      raw.analytics ?? defaultSiteSettings.analytics
    ),
    legal: parseSiteSettingGroup("legal", raw.legal ?? defaultSiteSettings.legal),
    footer: parseSiteSettingGroup("footer", raw.footer ?? defaultSiteSettings.footer),
    defaultCta: parseSiteSettingGroup(
      "defaultCta",
      raw.defaultCta ?? defaultSiteSettings.defaultCta
    ),
    system: parseSiteSettingGroup("system", raw.system ?? defaultSiteSettings.system),
