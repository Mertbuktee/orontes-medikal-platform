...
import { serviceListQuerySchema } from "@/lib/services/service-validation";
...
export default async function AdminServicesPage({
  ...
  const result = await repository.listAdminServices(filters);
  const canCreate = hasPermission(session.role, "services.create");
  const canUpdate = hasPermission(session.role, "services.update");
  const canPublish = hasPermission(session.role, "services.publish");
  const canReorder = hasPermission(session.role, "services.reorder");
  const canArchive = hasPermission(session.role, "services.delete");
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        title="Hizmetler"
        description="Web sitesinde sunulan teknik servis hizmetlerini yönetin."
      />
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm shadow-slate-200/60">
        <form
          ...
          className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.6fr_repeat(5,minmax(0,1fr))_auto]"
        >
        ...
        {result.items.length ? (
          result.items.map((service, index) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-sky-700">/{service.slug}</p>
                <p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-600">
                  {service.shortDescription}
                </p>
                <p className="mt-3 text-xs text-slate-500">
                  Güncelleyen: {service.updatedBy?.name ?? "-"} ·{" "}
                  {service.updatedAt.toLocaleDateString("tr-TR")}
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 lg:max-w-[270px] lg:justify-end">
                ...
                {canPublish && !service.archivedAt ? (
                  <>
                    ...
                    >
                      {service.isFeatured ? "Öne çıkarma" : "Öne çıkar"}
                    </ToggleButton>
                  </>
                ) : null}
                {canArchive ? (
                  service.archivedAt ? (
                    <ToggleButton action={restoreService} id={service.id} icon={RotateCcw}>
                      Geri al
                    </ToggleButton>
                  ) : (
                    <ToggleButton action={archiveService} id={service.id} icon={Archive}>
                      Arşivle
                    </ToggleButton>
                  )
                ) : null}
          ...
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              İlk hizmeti ekleyerek ana sayfa ve hizmetler sayfasını besleyin.
            </p>
          </div>
        )}
      </section>
...
type ServiceFilters = ReturnType<typeof serviceListQuerySchema.parse>;

function parseFilters(
  params: Record<string, string | string[] | undefined>
): ServiceFilters {
  return serviceListQuerySchema.parse({
...
function buildListHref(filters: ServiceFilters) {
