...
import { blogContentSchema, blogCategoryInputSchema, blogPostInputSchema } from "@/lib/blog/blog-validation";
...
export async function createBlogPost(formData: FormData) {
  ...
  await applyRequestedPostState(post.id, parsed.data.requestedStatus, parsed.data.input.scheduledFor);
...
export async function updateBlogPost(formData: FormData) {
  ...
  const repository = new PrismaBlogRepository(prisma);
  const post = await repository.updatePost(id, parsed.data.input, session.userId);
  await applyRequestedPostState(post.id, parsed.data.requestedStatus, parsed.data.input.scheduledFor);
...
export async function createBlogCategory(formData: FormData) {
  ...
  const category = await new PrismaBlogRepository(prisma).createCategory(parsed.data);
...
export async function updateBlogCategory(formData: FormData) {
  ...
  const category = await new PrismaBlogRepository(prisma).updateCategory(id, parsed.data);
...
export async function archiveBlogCategory(formData: FormData) {
  ...
  const category = await new PrismaBlogRepository(prisma).archiveCategory(parsed.data.id);
...
function parseBlogPostForm(formData: FormData) {
  ...
  const contentJson: unknown = JSON.parse(contentValue);
  const content = blogContentSchema.parse(contentJson);
